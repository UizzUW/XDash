...
namespace XDash
{
    public class MyNavigator : Navigator, INavigator
    {
        ...
        public MyNavigator(IDiContainer container,
            ILogger logger,
            IViewModelManager vmManager,
            IMessenger messenger) : base(container, logger, vmManager, messenger)
        {
        ...
        private async Task callOnNavigatedTo<TViewModel>(TViewModel viewModel, object parameter = null)
            where TViewModel : BaseViewModel
        {
            var ont = viewModel.GetType().GetTypeInfo().GetDeclaredMethods("OnNavigatedTo").First();
            var task = ont.Invoke(viewModel, new[] { parameter }) as Task;
            await task;
        ...
        private async Task callOnNavigatingFrom<TViewModel>(TViewModel viewModel, object parameter = null)
            where TViewModel : BaseViewModel
        {
            var onf = viewModel.GetType().GetTypeInfo().GetDeclaredMethods("OnNavigatingFrom").First();
            var task = onf.Invoke(viewModel, new[] { parameter }) as Task;
            await task;
